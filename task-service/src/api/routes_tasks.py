...
from src.db.models import Task, TaskComment, TaskEvaluation, UserPerformance, TaskStatus
...
async def update_task(task_id: int, payload: TaskUpdate, db: AsyncSession = Depends(get_db)):
    ...
    # Если меняется статус, обновляем соответствующие поля времени
...
async def delete_task(task_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # Публикуем событие удаления задачи
...
async def add_comment(task_id: int, payload: CommentCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Публикуем событие добавления комментария
...
async def evaluate_task(
    ...
    # Обновляем статистику производительности пользователя
    await update_user_performance(task.assignee_id, task.team_id, task.org_unit_id, db)

    # Публикуем событие оценки задачи
...
async def get_user_performance(
    ...
    # Получаем или создаем запись производительности
...
async def get_team_performance(
    ...
    """Получение статистики производительности команды"""
    if not period_start:
        today = date.today()
        quarter_start_month = 3 * ((today.month - 1) // 3)
        period_start = date(today.year, quarter_start_month + 1, 1)
...
async def get_org_unit_performance(
    ...
    """Получение статистики производительности подразделения"""
    if not period_start:
        today = date.today()
        quarter_start_month = 3 * ((today.month - 1) // 3)
        period_start = date(today.year, quarter_start_month + 1, 1)
    ...
    # Получаем всех участников подразделения
...
async def update_user_performance(
    ...
    """Обновление статистики производительности пользователя"""
    if not period_start:
        today = date.today()
        quarter_start_month = 3 * ((today.month - 1) // 3)
        period_start = date(today.year, quarter_start_month + 1, 1)
...
# src/api/routes_tasks.py (task-service)
@router.get("/performance/{user_id}/matrix")
async def get_performance_matrix(
        user_id: int,
        period: str = "quarter",  # quarter/month/year
        db: AsyncSession = Depends(get_db)
):
    """Получение матрицы оценок с сравнением"""
    # Определяем период
    today = date.today()
    if period == "quarter":
    ...
    # Получаем средние по команде/подразделению
...
async def calculate_average_metrics(
    ...
    """Расчёт средних метрик для команды/подразделения"""
